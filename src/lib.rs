...
#[cfg(not(feature = "32_bit_node_indices"))]
pub type NodeIndex = NonMaxU16;
#[cfg(feature = "32_bit_node_indices")]
pub type NodeIndex = NonMaxU32;

#[cfg(not(feature = "32_bit_node_indices"))]
pub type NodeIndexPrim = u16;
#[cfg(feature = "32_bit_node_indices")]
pub type NodeIndexPrim = u32;
...
impl Allocator {
    ...
    pub fn reset(&mut self) {
        ...
        self.free_nodes = (0..self.max_allocs)
            .map(|i| NodeIndex::try_from((self.max_allocs - i - 1) as NodeIndexPrim).unwrap())
            .collect();
    ...
    pub fn allocate(&mut self, size: u32) -> Option<Allocation> {
        ...
        let leaf_bin_index = match leaf_bin_index {
            ...
            None => {
                ...
                NonMaxU32::try_from(self.used_bins[top_bin_index.as_usize()].trailing_zeros())
                    .unwrap()
            }
        };
        ...
        let node = &mut self.nodes[node_index.as_usize()];
        ...
        if let Some(bin_list_next) = node.bin_list_next {
            self.nodes[bin_list_next.as_usize()].bin_list_prev = None;
        }
        ...
        if remainder_size > 0 {
            let Node {
                ...
                neighbor_next,
                ..
            } = self.nodes[node_index.as_usize()];
            ...
            if let Some(neighbor_next) = node.neighbor_next {
                self.nodes[neighbor_next.as_usize()].neighbor_prev = Some(new_node_index);
            }
            self.nodes[new_node_index.as_usize()].neighbor_prev = Some(node_index);
            self.nodes[new_node_index.as_usize()].neighbor_next = neighbor_next;
            self.nodes[node_index.as_usize()].neighbor_next = Some(new_node_index);
        }
    ...
    pub fn free(&mut self, allocation: Allocation) {
        ...
        if let Some(neighbor_prev) = self.nodes[node_index.as_usize()].neighbor_prev {
            if !self.nodes[neighbor_prev.as_usize()].used {
                // Previous (contiguous) free node: Change offset to previous
                // node offset. Sum sizes
                let prev_node = &self.nodes[neighbor_prev.as_usize()];
                ...
                self.nodes[node_index.as_usize()].neighbor_prev = prev_node.neighbor_prev;
            }
        }

        if let Some(neighbor_next) = self.nodes[node_index.as_usize()].neighbor_next {
            if !self.nodes[neighbor_next.as_usize()].used {
                // Next (contiguous) free node: Offset remains the same. Sum
                // sizes.
                let next_node = &self.nodes[neighbor_next.as_usize()];
                ...
                self.nodes[node_index.as_usize()].neighbor_next = next_node.neighbor_next;
            }
        }
        ...
        if let Some(neighbor_next) = neighbor_next {
            self.nodes[combined_node_index.as_usize()].neighbor_next = Some(neighbor_next);
            self.nodes[neighbor_next.as_usize()].neighbor_prev = Some(combined_node_index);
        }
        if let Some(neighbor_prev) = neighbor_prev {
            self.nodes[combined_node_index.as_usize()].neighbor_prev = Some(neighbor_prev);
            self.nodes[neighbor_prev.as_usize()].neighbor_next = Some(combined_node_index);
        }
    }
    ...
    fn insert_node_into_bin(&mut self, size: u32, data_offset: u32) -> NodeIndex {
        ...
        self.nodes[node_index.as_usize()] = Node {
            data_offset,
            data_size: size,
        ...
        if let Some(top_node_index) = top_node_index {
            self.nodes[top_node_index.as_usize()].bin_list_prev = Some(node_index);
        }
    ...
    fn remove_node_from_bin(&mut self, node_index: NodeIndex) {
        ...
        let node = self.nodes[node_index.as_usize()];

        match node.bin_list_prev {
            Some(bin_list_prev) => {
                // Easy case: We have previous node. Just remove this node from the middle of the list.
                self.nodes[bin_list_prev.as_usize()].bin_list_next = node.bin_list_next;
                if let Some(bin_list_next) = node.bin_list_next {
                    self.nodes[bin_list_next.as_usize()].bin_list_prev = node.bin_list_prev;
                }
            }
            None => {
    ...
    pub fn allocation_size(&self, allocation: Allocation) -> u32 {
        allocation
            .metadata
            .and_then(|metadata| self.nodes.get(NodeIndexPrim::from(metadata) as usize))
    ...
    pub fn storage_report_full(&self) -> StorageReportFull {
        ...
        for i in 0..NUM_LEAF_BINS {
            ...
            while let Some(node_index) = maybe_node_index {
                maybe_node_index =
                    self.nodes[NodeIndexPrim::from(node_index) as usize].bin_list_next;
                count += 1;
            }
